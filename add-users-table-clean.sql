-- Adicionar tabela de usuarios para o sistema GP Maquinas
-- Este arquivo deve ser executado apos a criacao do banco principal
...
-- Inserir usuarios das lojas (senhas criptografadas)
...
COMMENT ON COLUMN users.username IS 'Nome de usuario unico para login';
COMMENT ON COLUMN users.password_hash IS 'Hash da senha criptografada';
COMMENT ON COLUMN users.role IS 'Papel do usuario: admin ou store';
COMMENT ON COLUMN users.store_id IS 'ID da loja (apenas para usuarios store)';
COMMENT ON COLUMN users.full_name IS 'Nome completo do usuario';
COMMENT ON COLUMN users.is_active IS 'Se o usuario esta ativo no sistema';
